...
def detect_agent_intent(query: str) -> AgentIntent:
    ...
    if any(x in q for x in ["define", "meaning of", "what does"]):
        return AgentIntent.DEFINE

    if any(x in q for x in ["explain simply", "simple terms", "easy to understand"]):
        return AgentIntent.EXPLAIN_SIMPLE

    if any(x in q for x in ["summary", "overview", "tell me about", "key points", "takeaways"]):
...
class Responder:
    def __init__(self, retriever: Optional[Retriever] = None):
        self.retriever = retriever or Retriever()
        self.llm = genai.GenerativeModel("gemini-2.5-flash") if GEMINI_AVAILABLE else None
    ...
    def _build_prompt(
        ...
        style = {
            AgentIntent.SUMMARY: "Give a structured summary with headings and bullet points.",
            AgentIntent.EXPLAIN_SIMPLE: "Explain in very simple language, like teaching a beginner.",
            AgentIntent.DEFINE: "Give a short, clear definition first, then a brief explanation.",
            AgentIntent.PRECISE: "Answer clearly and accurately with technical correctness."
        }[intent]
...
"""
    ...
    def generate_response(self, query: str) -> Response:
        ...
        if not results:
            return Response(
                response_type=ResponseType.NO_RESULTS,
                answer="I couldn't find this information in the textbook.",
                citations=[],
                confidence=0.0
            )

        confidence = min(1.0, results[0].score / 10.0)

        if intent == AgentIntent.PRECISE and confidence < CONFIDENCE_THRESHOLD:
            return Response(
                response_type=ResponseType.LOW_CONFIDENCE,
                answer="The textbook does not fully support a precise answer to this question.",
                citations=self._format_citations(results[:1]),
                confidence=confidence,
        ...
        if self.llm:
            prompt = self._build_prompt(intent, query, results)
